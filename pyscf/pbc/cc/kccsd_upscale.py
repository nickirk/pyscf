...
from pyscf.pbc.cc import kintermediates_upscale as imdk_us
...
def update_amps(cc, t1, t2, eris, kikjka=[0, 0, 1]):
    ...
    if (nocc ** 4 * nkpts ** 3) * 16 / 1e6 + mem_now < cc.max_memory * .9:
        Woooo = imdk_us.cc_Woooo(t1, t2, eris, kconserv, ki=ki, kj=kj)
    else:
        fimd = lib.H5TmpFile()
        Woooo = fimd.create_dataset('oooo', (nkpts, nkpts, nkpts, nocc, nocc, nocc, nocc), t1.dtype.char)
        Woooo = imdk.cc_Woooo(t1, t2, eris, kconserv, Woooo, ki=ki, kj=kj)
    ...
    # TODO, check this function if any loops can be avoided.
    ki, kj, ka = kikjka
    # Chemist's notation for momentum conserving t2(ki,kj,ka,kb)
    kb = kconserv[ki, ka, kj]
    add_vvvv_(cc, t2new, t1, t2, eris, ka=ka, kb=kb)
    ...
    if (nocc ** 2 * nvir ** 2 * nkpts ** 3) * 16 / 1e6 * 2 + mem_now < cc.max_memory * .9:
        Wvoov = imdk_us.cc_Wvoov(t1, t2, eris, kconserv, ki=ki, ka=ka)
        Wvovo = imdk_us.cc_Wvovo(t1, t2, eris, kconserv, ki=ki, ka=ka)
    else:
        fimd = lib.H5TmpFile()
        ...
        Wvoov = imdk_us.cc_Wvoov(t1, t2, eris, kconserv, Wvoov, ki=ki, ka=ka)
        Wvovo = imdk_us.cc_Wvovo(t1, t2, eris, kconserv, Wvovo, ki=ki, ka=ka)
    ...
    # only update using residual at the selected k-points
    ki, kj, ka = kikjka
    kb = kconserv[ki, ka, kj]
    eia = _get_epq([0,nocc,ki,mo_e_o,nonzero_opadding],
                   [0,nvir,ka,mo_e_v,nonzero_vpadding],
    ...
    # copy the change to other k-points. TODO
    for ki_, kj_, ka_ in kpts_helper.loop_kkk(nkpts):
        if ki_ == ki and kj_ == kj and ka_ == ka:
            continue
        phase = t2[ki_, kj_, ka_] / np.abs(t2[ki_, kj_, ka_])
        t2new[ki_, kj_, ka_] = t2new[ki, kj, ka] * phase
...
def add_vvvv_(cc, Ht2, t1, t2, eris, kakb=[0,0]):
    nocc = cc.nocc
    nmo = cc.nmo
    nvir = nmo - nocc
    nkpts = cc.nkpts
    ...
    if cc.direct and getattr(eris, 'Lpv', None) is not None:
        ...
        _Wvvvv = imdk_us.cc_Wvvvv(t1, t2, eris, kconserv, ka=ka, kb=kb)
        ...
        _Wvvvv = imdk_us.cc_Wvvvv(t1, t2, eris, kconserv, _Wvvvv, ka=ka, kb=kb)
