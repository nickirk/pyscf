...
'''
RCCSD for non-hermitian real integrals
2-fold permutation symmetry has been used
(ij|kl) = (kl|ij)
'''
...
def _contract_s4vvvv_t2(mycc, mol, vvvv, t2, out=None, verbose=None):
    ...
    def contract_blk_(eri, i0, i1, j0, j1):
        ...
        if t2.dtype == numpy.double:
            ...
            Ht2[:,j0:j1] += lib.einsum('xef,efab->xab', x2[:,i0:i1], eri)
        else:
        ...
        if i0 > j0:
            ...
            if t2.dtype == numpy.double:
                ...
                Ht2[:,i0:i1] += lib.einsum('xef,abef->xab', x2[:,j0:j1], eri)
            else:
                raise NotImplementedError('t2.dtype %s not supported!' % t2.dtype)
...
class CCSDBase(lib.StreamObject):
    ...
    def __init__(self, mf, frozen=None, mo_coeff=None, mo_occ=None):
        ...
        self.eri_h5file = 'xtc_fcidump.h5'
...
def _make_eris_outcore(mycc, mo_coeff=None):
    ...
    eris.vvvo = eris.feri1.create_dataset('vvvo', (nvpair,nvir,nocc), 'f8')
    def map_tc_eri(feri, eris):
        """
        This function maps the xtc fcidump hdf5 file to 
        the eris object.
        """
        return eris
    eris = map_tc_eri(mycc.eri_h5file, eris)
