...
#         Ke Liao <ke.liao.whu@gmail.com>
#

'''
RCCSD intermediates without using any symmetry.
Equations taken from Hirata et al., J. Chem. Phys. 120, 2581 (2004).
'''
...
def cc_Wvvvv(t1, t2, eris):
    '''Build Wvvvv without assuming any symmetry'''
    eris_ovvv = np.asarray(eris.get_ovvv())
    Wabcd = lib.einsum('kdac,kb->abcd', eris_ovvv, -t1)
    Wabcd += lib.einsum('kcbd,ka->abcd', eris_ovvv, -t1)
    Wabcd += np.asarray(eris.get_vvvv())
    return Wabcd

def cc_Wvoov(t1, t2, eris):
    '''Build Wvoov without assuming any symmetry'''
    eris_ovvv = np.asarray(eris.get_ovvv())
    eris_ovoo = np.asarray(eris.ovoo)
    ...
    eris_ovov = np.asarray(eris.ovov)
    Wakic -= lib.einsum('ldkc,ilda->akic', eris_ovov, t2)
    return Wakic

def cc_Wvovo(t1, t2, eris):
    '''Build Wvovo without assuming any symmetry'''
    eris_ovvv = np.asarray(eris.get_ovvv())
    eris_ovoo = np.asarray(eris.ovoo)
...
def Wvvvo(t1, t2, eris, _Wvvvv=None):
    '''Build Wvvvo without assuming any symmetry'''
    nocc,nvir = t1.shape
    eris_ovvv = np.asarray(eris.get_ovvv())
    Wabcj = lib.einsum('ldac,ljdb->abcj', eris_ovvv, t2)
    ...
    if np.any(t1):
        if _Wvvvv is None:
            _Wvvvv = cc_Wvvvv(t1, t2, eris)
        Wabcj += lib.einsum('abcd,jd->abcj', _Wvvvv, t1)
    return Wabcj

def Wovoo(t1, t2, eris):
    '''Build Wovoo without assuming any symmetry'''
    eris_ovoo = np.asarray(eris.ovoo)
    eris_ovvv = np.asarray(eris.get_ovvv())
    Wkbij = lib.einsum('ldki,ljdb->kbij', eris_ovoo, t2)
